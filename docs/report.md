...
从评分项看，本项目的交付物与课程要求逐项对应：选题与设计思想对应研究生教务资料分散、答案需可追溯的问题；Specs 对应 `docs/product_spec.md`、`docs/architecture_spec.md` 和 `docs/api_spec.md`；系统架构对应 Agentic RAG 闭环、工具注册、数据库和 Web 工作台；关键实现对应 `EduRagAgent` 状态机、受控工具、DeepSeek/OpenAI 兼容生成和 MCP Server；测试评估对应 pytest、25 条评估集、来源命中率和 DeepSeek 联调结果；扩展章节则说明向真实向量库、LangGraph、Trace 可视化和云部署演进的路径。
...
![图 1 EduRAG-Agent 核心架构](assets/architecture_overview.png)
...
Web 工作台由 `app/main.py` 提供，支持注册登录、多轮会话、知识库浏览、来源详情、系统统计和办事记录。当前项目已经完成真实页面联调，运行 `uvicorn app.main:app --reload --port 8000` 后可在 `http://127.0.0.1:8000` 打开工作台。页面顶部会显示 DeepSeek API 状态、当前用户和功能入口；智能问答区展示模型、耗时、上下文数量和回答内容；来源区和知识详情页用于复核检索证据；“我的待办”页面与聊天工具共享同一套 SQLite 数据。
...
![图 2 AI IDE 中使用 Codex 辅助开发与报告整理](assets/ai_ide_usage.png)
...
![图 3 DeepSeek API 智能问答页](assets/demo_ask_deepseek.png)

图 3 展示完整问答链路：用户提问“报告 PDF 必须包含哪些章节？”，系统通过 DeepSeek API 生成答案，并显示模型来源、耗时和上下文数量。回答内容对应课程要求中的七个章节、封面字段和 PDF 导航要求。

![图 4 本轮检索来源](assets/demo_sources.png)

图 4 对应 RAG 的证据约束能力。页面展示命中资料、分类、标题、摘要和相似度，便于确认答案来自课程要求、GitHub 仓库要求和报告章节结构等预期来源。

![图 5 知识详情页](assets/demo_source_detail.png)

图 5 展示来源可复核能力。评阅者可以从检索来源进入知识详情页，查看“报告要求”的完整内容，并继续基于该资料追问。

![图 6 聊天触发数据库动作](assets/demo_chat_action.png)

![图 7 我的办事待办页面](assets/demo_todo_page.png)

图 6 和图 7 对应工具调用与数据库动作能力。用户通过聊天输入自然语言后，Agent 将意图路由到受控工具；待办页面读取同一个 `service_requests` 表，因此聊天创建的记录会同步出现在页面中。
...
![图 8 DeepSeek API 联调结果](assets/deepseek_eval.png)
